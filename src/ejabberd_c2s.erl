...
-export([tls_options/1, tls_required/1, tls_verify/1, tls_enabled/1,
     ...
     handle_unauthenticated_packet/2, handle_authenticated_packet/3,
...
handle_authenticated_packet(_El, Pkt, #{lserver := LServer} = State) when not ?is_stanza(Pkt) ->
    ejabberd_hooks:run_fold(c2s_authenticated_packet,
                LServer, State, [Pkt]);
handle_authenticated_packet(El, Pkt, #{lserver := LServer, jid := JID,
                   ip := {IP, _}} = State) ->
    Pkt1 = xmpp:put_meta(Pkt, ip, IP),
    State1 = ejabberd_hooks:run_fold(c2s_authenticated_packet,
                     LServer, State, [Pkt1]),
    #jid{luser = LUser} = JID,
    {El2, Pkt2, State2} = ejabberd_hooks:run_fold(
               user_send_packet, LServer, {El, Pkt1, State1}, []),
    case Pkt2 of
    drop ->
        State2;
    ...
    _ ->
        check_privacy_then_route(State2, El2, Pkt2)
    end.
...
%% lifang update
-spec check_privacy_then_route(state(), any(), stanza()) -> state().
check_privacy_then_route(#{lang := Lang} = State, El2, Pkt) ->
    case privacy_check_packet(State, Pkt, out) of
        deny ->
            ErrText = <<"Your active privacy list has denied "
            "the routing of this stanza.">>,
