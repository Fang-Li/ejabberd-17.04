...
%%  end.
%%test_two(N)->
%%  Table = <<"test_mongodb_table">>,
%%  Data = {id,1,teststr,{a,123,b,<<"123123">>}},
%%  {ok,Connect} = mongo_pool:checkout(?MONGO_POOL),
%%  mongo:insert(Connect, Table, [Data]),
%%  Index = {'teststr.a',123},
%%  Result = mongo:find_one(Connect, Table, Index,{teststr,true}),
%%  ?WARNING_MSG("result~p",[Result]),
%%  Update  ={'$set',{'teststr.b',erlang:integer_to_binary(N)}},
%%  mongo:update(Connect, Table, Index, Update).
%%
%%%%测试添加规划用户
%%test_add_to_index(0)->
%%  skip;
%%test_add_to_index(N) when N > 0->
%%  Table = <<"onlineuser">>,
%%  Index = {lid,1},
%%  Update = {'$push',{loginid,N}},
%%  {ok,Connect} = mongo_pool:checkout(?MONGO_POOL),
%%  mongo:update(Connect, Table, Index, Update),
%%  test_add_to_index(N-1).
%%
%%
...
read_opt_uidlist()->    
  ...
  try
    ...
    _:Reason ->
      ...
      ?ERROR_MSG("mongo db opt uidlist data error!!! ~n~p~n~p",[Reason,Err]),
...
handle_cast({sysn_write,Data},State)->
  {ok,ConnectPid} = mongo_pool:checkout(?MONGO_POOL),
  Conf = ejabberd_config:get_option({mongodb,?MYNAME},fun(V)->V end),
